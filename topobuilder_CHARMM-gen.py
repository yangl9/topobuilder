...
for line in range(nlinea):
	atomff[line][6] = atomff[line][6].ljust(7)
...
#!      put the connecting atoms of the line_th atom to an list
		for nag in range(2,nele):
			...
			if int(content[line][nag]) > cb2: #! avoid double-counted
				...
				s = atomff[i1-1][6] + atomff[i2-1][6] 
				sr = atomff[i2-1][6] + atomff[i1-1][6] 
				so = ""
			...
			if so == '': #! use the artificial bond info instead
				for j in range(ub):
					if markb1[j] == l[ca][1]: 
						...
						sr2 = ""
						if l[ca][0] == markb2[j]:
							s2 = inb2[j] + inb1[j] + atomff[l[ca][2]-1][6]
							sr2 = atomff[l[ca][2]-1][6] + inb1[j] + inb2[j]
						elif l[ca][2] == marka2[j]:
							s2 = atomff[l[ca][0]-1][6] + inb1[j] + inb2[j]
							sr2 = inb2[j] + inb1[j] + atomff[l[ca][0]-1][6]
#						if so == '': #! if even this does not help, use two artificial bonds
					elif markb2[j] == l[ca][1]:
						if l[ca][0] == markb1[j]:
							s2 = inb1[j] + inb2[j] + atomff[l[ca][2]-1][6]
							sr2 = atomff[l[ca][2]-1][6] + inb2[j] + inb1[j]
						elif l[ca][2] == markb1[j]:
							s2 = atomff[l[ca][0]-1][6] + inb2[j] + inb1[j]
							sr2 = inb1[j] + inb2[j] + atomff[l[ca][0]-1][6]
					for lnff in range (816,2457):
...
for k in range(ca): 
	if l[k][0] > l[k][1]: #! avoid double-counted
		...
		for nag in range(2,nele):
			...
			sr = ""
			if d1 != l[k][1]:
				s = atomff[d1-1][6] + atomff[l[k][0]-1][6] + atomff[l[k][1]-1][6] + atomff[l[k][2]-1][6]
				sr = atomff[l[k][2]-1][6] + atomff[l[k][1]-1][6] + atomff[l[k][0]-1][6] + atomff[d1-1][6]
				so = ""
				...
				if so == '': #! use the artificial bond info instead
					for j in range(ua):
						...
						if marka0[j] == l[k][0] and marka1[j] == l[k][1] and marka2[j] == l[k][2]:
							s2 = atomff[d1-1][6] + ina1[j] + ina2[j] + ina3[j]
							sr2 = ina3[j] + ina2[j] + ina1[j] + atomff[d1-1][6]
						for lnff in range (2458,6594):
								if s2 != "" and re.search(s2, ffinfo[lnff]):
									so = ffinfo[lnff] + "                 ??? SATISFIED ??? "
								elif sr2 != "" and re.search(sr2, ffinfo[lnff]):
						...
						if so == '':
							if marka0[j] == d1 and marka1[j] == l[k][0] and marka2[j] == l[k][1]:
								s2 = ina1[j] + ina2[j] + ina3[j] + atomff[l[k][2]-1][6]
								sr2 = atomff[l[k][2]-1][6] + ina3[j] + ina2[j] + ina1[j]
							elif marka0[j] == l[k][1] and marka1[j] == l[k][0] and marka2[j] == d1:
								s2 = ina3[j] + ina2[j] + ina1[j] + atomff[l[k][2]-1][6]
								sr2 = atomff[l[k][2]-1][6] + ina1[j] + ina2[j] + ina3[j]
							for lnff in range (2458,6594):
								if s2 != "" and re.search(s2, ffinfo[lnff]):
									so = ffinfo[lnff] + "                  ??? SATISFIED ??? "
								elif sr2 != "" and re.search(sr2, ffinfo[lnff]):
	...
	if l[k][2] > l[k][1]:
		...
		for nag in range(2,nele):
			...
			if d4 != l[k][1]:
				s = atomff[l[k][0]-1][6] + atomff[l[k][1]-1][6] + atomff[l[k][2]-1][6] + atomff[d4-1][6]
				sr = atomff[d4-1][6] + atomff[l[k][2]-1][6] + atomff[l[k][1]-1][6] + atomff[l[k][0]-1][6]
				so = ""
				for lnff in range (2458,6594):
					if s != "" and re.search(s, ffinfo[lnff]):
						so = ffinfo[lnff]
				...
				if so == '': #! use the artificial bond info instead
					for j in range(ua):
						...
						sr2 = ""
						if marka0[j] == l[k][0] and marka1[j] == l[k][1] and marka2[j] == l[k][2]:
							s2 = ina1[j] + ina2[j] + ina3[j] + atomff[d4-1][6]
							sr2 = atomff[d4-1][6] + ina3[j] + ina2[j] + ina1[j]
						for lnff in range (2458,6594):
								if s2 != "" and re.search(s2, ffinfo[lnff]):
									so = ffinfo[lnff] + "                   ??? SATISFIED ??? "
								elif sr2 != "" and re.search(sr2, ffinfo[lnff]):
						...
						if so == '':
							if marka0[j] == l[k][1] and marka1[j] == l[k][2] and marka2[j] == d4:
								s2 = atomff[l[k][0]-1][6] + ina1[j] + ina2[j] + ina3[j]
								sr2 = ina3[j] + ina2[j] + ina1[j] + atomff[l[k][0]-1][6]
							elif marka0[j] == d4 and marka1[j] == l[k][2] and marka2[j] == l[k][1]:
								s2 = atomff[l[k][0]-1][6] + ina3[j] + ina2[j] + ina1[j]
								sr2 = ina1[j] + ina2[j] + ina3[j] + atomff[l[k][0]-1][6]
							for lnff in range (2458,6594):
								if s2 != "" and re.search(s2, ffinfo[lnff]):
									so = ffinfo[lnff] + "                   ??? SATISFIED ??? "
								elif sr2 != "" and re.search(sr2, ffinfo[lnff]):
...
for k in range(ca): #! repeating
	...
	for nag in range(2,nele):
		...
		if im4 > l[k][0] and im4 > l[k][2]: #! avoid double-counted, s=1234 sr=1324
			s = atomff[l[k][0]-1][6] + atomff[l[k][1]-1][6] + atomff[l[k][2]-1][6] + atomff[im4-1][6]
			sr = atomff[l[k][0]-1][6] + atomff[l[k][2]-1][6] + atomff[l[k][1]-1][6] + atomff[im4-1][6]
			so = ""
